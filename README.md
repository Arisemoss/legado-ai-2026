...
### 修复（2026-09 第十批 · AI 层自查）
- 🛡 **后台确认不再丢卡**：写操作确认增加 sticky 槽位（离开页面回来后仍能确认）；并行工具卡不再互相覆盖（原先 4 个工具只显示 1 张）。
- ⏹ **停止语义**：流式输出中途停止会正确标记「已停止」。
- 🫧 **悬浮球定位**：修掉父容器未测量时可能定位到屏幕外的问题。
- ⚙️ **默认值一致性**：「流式输出」默认值与设置页显示一致；删除/清空会话前先停任务，避免孤儿消息。
- 🔒 **书源写入加固**：URL 字段拒绝 javascript / file / content / data 等危险 scheme。

### 修复（2026-09 第十一批 · 真机反馈）
- 📦 **书源集合上限 2MB → 16MB**（替换规则 8MB），大集合不再被误判失败。
- 🩺 **检测入口迁移到「书源管理」右上角**；检测并发 4 → 8、结果逐条实时回填、右上角可直接看运行日志。
- 🐞 **修掉检测卡死**：检测的规则解析原先跑在主线程（295 个源时界面无响应、停止也卡）→ 整体移到后台线程并分块执行，可随时停止。
- 💰 **单轮 token 预算默认 64k**（原 32k）。
